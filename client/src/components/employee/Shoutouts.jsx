...
export default function Shoutouts() {
  ...
  // Get my shoutouts and received shoutouts for sidebar
  const myShoutouts = allShoutouts
    .filter((s) => s.sender_id === CURRENT_USER_ID)
    .slice(-3)
    .reverse();
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-start justify-center p-6">
      <div className="w-full max-w-5xl grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-3xl shadow-lg p-6 border border-slate-100">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-lg font-semibold">All Shoutouts</h3>
            </div>

            {loading && <p className="text-sm text-slate-500">Loading...</p>}
            {!loading && allShoutouts.length === 0 && (
        ...
        {/* Right pane: received shoutouts and my shoutouts */}
        <div className="space-y-4">
          {/* Received Shoutouts */}
          <div className="bg-white rounded-3xl shadow-lg p-6 border border-slate-100">
            <div className="flex items-center justify-between mb-3">
          ...
          <div className="bg-white rounded-3xl shadow-lg p-6 border border-slate-100">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-lg font-semibold">My Shoutouts</h3>
            </div>
            {myShoutouts.length === 0 ? (
              <p className="text-sm text-slate-500">No shoutouts sent yet.</p>
            ) : (
              <div className="mt-3 space-y-4">
                {myShoutouts.map((s) => (
                  <div key={s.id} className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <div className="font-medium text-sm">{s.receiver_name}</div>
                      <div className="text-xs text-slate-600">{s.message}</div>
                    </div>
                  </div>
                ))}
