...
const Analytics = () => {
  ...
  return (
    <div className="p-6 bg-[#f0f6ff] min-h-screen">
      ...
      {/* HEADER */}
      <div className="bg-white p-6 rounded-lg shadow-sm border mb-6 flex flex-wrap gap-3 items-center">

        <h1 className="text-2xl font-bold mr-4">Admin Dashboard Analytics</h1>
        ...
        {["week", "month", "quarter", "year"].map((r) => (
          <button
            ...
            className={`px-4 py-2 rounded text-sm ${
              range === r ? "bg-blue-600 text-white" : "bg-gray-200 text-gray-700"
            }`}
          >
            {r.toUpperCase()}
          </button>
        ...
        >
          <option value="donut">Top Contributors (Donut)</option>
          <option value="pie">Most Tagged (Pie)</option>
          <option value="bar">Departments (Bar)</option>
          <option value="line">Weekly Trend (Line)</option>
          <option value="trend">Advanced Trend</option>

          {/* ⭐ ADDED ALL CHARTS OPTION */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6 w-full">
        ...
        <div className="bg-white p-8 rounded-lg shadow-sm border text-center">
          <p className="text-xl font-bold text-blue-600">{current.shoutoutsSent}</p>
          <p className="text-xl text-gray-700 font-medium mt-2">Shout-outs Sent</p>
        </div>

        <div className="bg-white p-8 rounded-lg shadow-sm border text-center">
          <p className="text-xl font-bold text-green-600">{current.reactionsReceived}</p>
          <p className="text-xl text-gray-700 font-medium mt-2">Reactions Received</p>
        </div>

        <div className="bg-white p-8 rounded-lg shadow-sm border text-center">
          <p className="text-xl font-bold text-purple-600">{current.commentsMade}</p>
          <p className="text-xl text-gray-700 font-medium mt-2">Comments Made</p>
        </div>
      ...
      <div className="w-full flex justify-center">
        <div className="bg-white p-6 rounded-lg shadow-sm border w-full max-w-4xl mx-auto">
