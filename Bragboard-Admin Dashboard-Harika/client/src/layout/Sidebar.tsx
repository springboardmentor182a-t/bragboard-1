...
export function AppSidebar() {
  ...
  return (
    <Sidebar collapsible="icon" className="border-r border-sidebar-border shadow-2xl animate-in fade-in duration-500">
      <SidebarContent>
        <div className="px-6 py-4 border-b border-sidebar-border h-[65px] flex items-center relative overflow-hidden">
          <h1 className="font-bold text-xl text-sidebar-foreground truncate transition-opacity duration-200 opacity-100 group-data-[state=collapsed]:opacity-0">
            BragBoard
          </h1>
          <div className="absolute inset-0 flex items-center justify-center opacity-0 transition-opacity duration-200 group-data-[state=collapsed]:opacity-100 pointer-events-none">
            <span className="text-sidebar-primary font-bold text-2xl">B</span>
          </div>
        </div>

        <SidebarGroup>
          <SidebarGroupLabel className="transition-opacity duration-200 group-data-[state=collapsed]:opacity-0">Dashboard</SidebarGroupLabel>
          <SidebarGroupContent>
            <SidebarMenu>
              {menuItems.map((item) => (
